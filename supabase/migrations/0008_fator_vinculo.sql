-- 0008_fator_vinculo.sql — fator de conversão por vínculo (cProd)
-- Ex.: parafuso vem em CENTO na nota; fator=100 → custo_real_unidade = custo_nota / 100.
-- Fica memorizado por código (cProd), valendo para todas as notas daquele código.

alter table public.vinculos_cprod add column if not exists fator_conversao numeric;

-- api_precos: aplica o fator do vínculo (divisão) no custo, com prioridade sobre a
-- conversão por unidade do produto (multiplicação). Vale para comprados e para os
-- componentes dos montados.
